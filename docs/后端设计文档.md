...
#### 问题 1: 答完最后一题后会话状态仍为 IN_PROGRESS

| 项目 | 内容 |
|:---|:---|
| **问题描述** | 用户答完所有题目后（`currentQuestionNo == totalQuestions`），会话状态仍然显示 `IN_PROGRESS` 而非 `COMPLETED` |
| **影响范围** | 前端无法正确判断测验是否完成，用户体验受损 |
| **根本原因** | `submitAnswer` 方法中，最后一题的状态更新依赖 `agentResponse.isShouldFinish()` 返回值，而该方法可能返回 `false` |
| **修复方案** | 当 `hasNext == false` 时（即最后一题已答完），直接设置 `session.setStatus(QuizStatus.COMPLETED)` |
| **修复文件** | `QuizSessionServiceImpl.submitAnswer()` |

**修复前代码**:
```java
} else {
    // 判断是否需要生成更多题目或结束
    if (agentResponse.isShouldFinish()) {  // ← 问题: 可能返回 false
...
WHERE status = 'IN_PROGRESS' 
  AND current_question_no >= total_questions 
  AND total_questions > 0;
```

---

#### 问题 2: 答完所有题目后调用 `/next` 接口返回重复题目

| 项目 | 内容 |
|:---|:---|
| **问题描述** | 用户答完所有题目后，调用 `GET /api/v1/quiz/session/{id}/next` 仍返回最后一题 |
| **影响范围** | 前端可能循环调用，用户体验混乱 |
| **根本原因** | `getNextQuestion` 方法未检查所有题目是否已答完 |
| **修复方案** | 在返回题目前检查 `currentQuestionNo >= totalQuestions` 且已答题数 >= 总题数时，抛出明确异常 |
| **修复文件** | `QuizSessionServiceImpl.getNextQuestion()` |

**修复后代码**:
```java
// 检查是否已经是最后一题且已答过
if (session.getCurrentQuestionNo() >= session.getTotalQuestions()) {
    long answeredCount = responseRepository.countActiveBySessionId(sessionId);
    if (answeredCount >= session.getTotalQuestions()) {
        throw new BusinessException(ErrorCode.OPERATION_ERROR, "所有题目已答完，请查看结果");
    }
}
```
...
#### 问题 3: 简答题导致 Agent 死循环

| 项目 | 内容 |
|:---|:---|
| **问题描述** | 处理简答题/解释题时，Agent 不断重复调用 `AnswerEvaluator` 工具，触发死循环检测 |
| **影响范围** | 用户提交答案后无法获得响应，服务资源浪费 |
| **根本原因** | 1. `DecisionEngine` 未将题目信息传递给 `AnswerEvaluator`，导致 `questionType=null`<br>2. `AnswerEvaluatorTool` 返回的 `ToolResult` 未设置 `terminal` 标记 |
| **修复方案** | 1. 通过 `AgentContext.extras` 传递题目信息<br>2. `AnswerEvaluatorTool` 返回 `ToolResult.successAndTerminal()` |
| **修复文件** | `QuizSessionServiceImpl`, `DecisionEngine`, `AnswerEvaluatorTool` |

---

### 11.3 Hibernate/JPA 问题

#### 问题 4: MultipleBagFetchException

| 项目 | 内容 |
|:---|:---|
| **问题描述** | 使用 `@EntityGraph(attributePaths = {"questions", "responses"})` 同时加载两个集合时抛出异常 |
| **错误信息** | `org.hibernate.loader.MultipleBagFetchException: cannot simultaneously fetch multiple bags` |
| **根本原因** | Hibernate 不支持同时 eager fetch 多个 `List` 类型的集合 |
| **修复方案** | 分步查询，不同时加载多个集合 |
| **修复文件** | `QuizSessionServiceImpl.getSessionDetail()` |
